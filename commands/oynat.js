...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!args.length) {
      return message.channel.send("> :beginner: **Şarkı ismi veya link girmelisin.**");
    }
    
    const { channel } = message.member.voice;
    if (!channel) {
      
      return message.channel.send("> :beginner: **Herhangi bir ses kanalına girmelisin.**");
    }
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      return message.channel.send("> :beginner: **Oynatma listesi oynatılamıyor.**");
    }
    ...
    if (urlcheck) {
      try {
         ...
         if(!result[0]) return message.channel.send('> :beginner: **Yanlış link.**')
        ...
        if (message.include === "copyright") {
          return message
            .reply("> :beginner: **Bu video telif hakları nedeni ile oynatılamıyor.**")
            .catch(console.error);
        } else {
          console.error(error);
        }
      ...
      try {
        ...
        if(!result[0]) return message.channel.send('> :beginner: **Arama sonucu bulunamadı.**')
        songData = await ytdl.getInfo(result[0].url)
         song = {
           title: songData.videoDetails.title,
           url: songData.videoDetails.video_url,
    ...
    if(serverQueue) {
      ...
      return serverQueue.textChannel.send( new Discord.MessageEmbed()//It is encoded by K4H1N
        ...
        .addField('> :beginner: Kanal',song.author,false)
        .addField('> :beginner: Şarkı Saniyesi',song.duration,false)
        .addField('> :beginner: İzlenme Sayısı',song.wiews.toLocaleString(),false)
        .addField('> :beginner: Like',song.likes.trues,false)
        .addField('> :beginner: Dislike',song.likes.falses,false))
      .catch(console.error)
    } else {
